#!/usr/bin/env python3
"""Download postimg URLs and replace them with data:image/png;base64 URIs.

Mirrors GoogleSnakePudding/tools/embed_postimg.py for Remix sources and for
MoreMenu/Visibility copies pulled during RemixBuilder.
"""
...
# postimg CDN often serves an expired cert; still fetch the image bytes.
SSL_CTX = ssl._create_unverified_context()

DEFAULT_SCAN = [
    ROOT / "src" / "BurgerInit.js",
    ROOT / "src" / "CandyInit.js",
    ROOT / "src" / "ChessInit.js",
    ROOT / "src" / "CustomFruitInit.js",
    ROOT / "src" / "CustomFruitPresets.js",
    ROOT / "src" / "UltraPlaceInit.js",
]
...
def fetch_png(url: str) -> tuple[bytes, str]:
    ...
    if last_err is not None:
        raise RuntimeError(
            f"Failed fetching {url} after {MAX_RETRIES} tries: {last_err}"
        ) from last_err
...
def embed_files(files: list[Path]) -> int:
    files = [p for p in files if p.is_file()]
    if not files:
        log("No source files to scan")
        return 0
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Embed i.postimg.cc PNG URLs as data:image/png;base64"
    )
    parser.add_argument(
        "paths",
        nargs="*",
        type=Path,
        help="Files to scan/replace (default: Remix src modules with postimg)",
    )
    args = parser.parse_args()
    files = [p.resolve() for p in args.paths] if args.paths else DEFAULT_SCAN
    embed_files(files)
